...
export async function triggerEmailDecision(leadId, triggerEvent, extraContext = {}) {
  try {
    ...
    // 2. Check global and per-rule toggles from admin_settings
    const { data: settingsRows } = await supabaseAdmin
      .from("admin_settings")
      .select("key, value")
      .in("key", ["email_global_enabled", `email_rule_${triggerEvent}`]);

    const settingsMap = Object.fromEntries((settingsRows || []).map((r) => [r.key, r.value]));

    // Global off → skip silently (intentional pause, no need to log)
    if (settingsMap["email_global_enabled"] === "false") return;

    // Rule off → log so admin can see it was evaluated but suppressed
    if (settingsMap[`email_rule_${triggerEvent}`] === "false") {
