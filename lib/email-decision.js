...
async function logOutcome(leadId, triggerEvent, status, { subject, body, ai_reasoning, error_message, rule_id, rule_name } = {}) {
  try {
...
export async function triggerEmailDecision(leadId, triggerEvent, extraContext = {}) {
  const { rule_id, rule_name } = extraContext;
  const logCtx = { rule_id, rule_name };
  const isRuleTrigger = triggerEvent.startsWith("rule:");
  ...
  try {
    ...
    // Global off → skip silently
    if (emailSettings && emailSettings.global_enabled === false) return;

    // System rule toggle check (only for non-rule-builder triggers)
    if (!isRuleTrigger) {
    ...
    // 5. Build AI prompt
    const triggerLabel = rule_name
      ? `Matched custom rule: "${rule_name}"`
      : TRIGGER_LABELS[triggerEvent] || triggerEvent;
