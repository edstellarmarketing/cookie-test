...
export default function LeadsPage() {
  ...
  const deleteLead = async (lead, e) => {
    e.stopPropagation();
    if (!confirm(`Delete "${lead.name}" (${lead.email})? This removes all their visits, milestones, and cart data.`)) return;
                ...
                <tbody>
                  {leads.map((lead, index) => (
                    ...
                    >
                      ...
                      <td className="py-3 pr-4 text-gray-500 whitespace-nowrap">{formatDate(lead.created_at)}</td>
                      <td className="py-3">
                        <button
